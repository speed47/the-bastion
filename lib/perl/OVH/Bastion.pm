...
sub is_account_ttl_nonexpired {
    ...
    if ($fnret) {
        my $ttl = $fnret->value;
        if ($ttl !~ /^[0-9]+$/) {
            warn_syslog("Invalid TTL value '$ttl' for account '$sysaccount', denying access");
        ...
        if ($created !~ /^[0-9]+$/) {
            warn_syslog("Invalid account creation time '$created' for account '$sysaccount', denying access");
            return R('ERR_INVALID_TTL',
                msg => "Issue with your account TTL configuration, access denied. "
                  . "Please check with an administrator");
        }
        ...
        if ($expiryTime < time()) {
            ...
            return R(
                'KO_TTL_EXPIRED',
                msg   => "Sorry $account, your account TTL has expired since " . $fnret->value->{'human'},
